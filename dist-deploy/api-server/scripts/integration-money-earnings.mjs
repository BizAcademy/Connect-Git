...
var require_sonic_boom = __commonJS({
  "../../node_modules/.pnpm/sonic-boom@4.2.1/node_modules/sonic-boom/index.js"(exports, module) {
    ...
    var [major, minor2] = (process.versions.node || "0.0").split(".").map(Number);
    var kCopyBuffer = major >= 22 && minor2 >= 7;
...
// src/scripts/integration-money-earnings.ts
...
// src/lib/deposits.ts
var minor = (amount) => Math.round(amount * 100);
async function createPayment(input) {
  ...
  await getMysqlPool().execute(
    ...
    [
      ...
      input.userId,
      minor(input.amount),
      minor(input.feeAmount ?? 0),
      input.chargeAmount == null ? null : minor(input.chargeAmount),
      input.currency,
      input.orderId,
      input.country,
      input.operator,
...
async function findEarning(providerOrderId2, provider) {
  const sql = provider === void 0 ? "SELECT * FROM earnings WHERE provider_order_id=? LIMIT 1" : "SELECT * FROM earnings WHERE provider_order_id=? AND provider=? LIMIT 1";
  const [rows] = await getMysqlPool().execute(sql, provider === void 0 ? [providerOrderId2] : [providerOrderId2, provider]);
...
if (required.some((key) => process.env[key] === void 0)) throw new Error("MYSQL_* test configuration is incomplete");
var suffix = crypto2.randomUUID();
var userId = crypto2.randomUUID();
var paymentOrderId = `integration-${suffix}`;
var providerOrderId = `provider-${suffix}`;
var pool2 = mysql2.createPool({ host: process.env.MYSQL_HOST, port: Number(process.env.MYSQL_PORT || 3306), database: process.env.MYSQL_DATABASE, user: process.env.MYSQL_USER, password: process.env.MYSQL_PASSWORD });
try {
  await pool2.execute("INSERT INTO users (id,email,password_hash) VALUES (?,?,?)", [userId, `${suffix}@integration.invalid`, "not-a-login"]);
  await pool2.execute("INSERT INTO profiles (user_id,email,username) VALUES (?,?,?)", [userId, `${suffix}@integration.invalid`, `it-${suffix.slice(0, 12)}`]);
  ...
  const [paymentRows] = await pool2.execute("SELECT amount_minor,fee_minor,charge_minor FROM payments WHERE id=?", [paymentId]);
  const payment = paymentRows[0];
  if (!payment || Number(payment.amount_minor) !== 5e4 || Number(payment.fee_minor) !== 1e3 || Number(payment.charge_minor) !== 51e3) throw new Error("payment minor-unit persistence assertion failed");
  const rec = { ts: (/* @__PURE__ */ new Date()).toISOString(), provider_order_id: providerOrderId, user_id: userId, service: 1, service_name: "integration", quantity: 1e3, rate_usd: 0, user_price_fcfa: 500, provider_cost_usd: 0, provider_cost_fcfa: 400, gain_fcfa: 100, provider: 1 };
  await appendEarning(rec);
  await appendEarning(rec);
  const [countRows] = await pool2.execute("SELECT COUNT(*) count FROM earnings WHERE provider=? AND provider_order_id=?", [1, providerOrderId]);
  ...
  await pool2.execute("DELETE FROM earnings WHERE provider_order_id=?", [providerOrderId]).catch(() => void 0);
  await pool2.execute("DELETE FROM payments WHERE order_id=?", [paymentOrderId]).catch(() => void 0);
  await pool2.execute("DELETE FROM profiles WHERE user_id=?", [userId]).catch(() => void 0);
  await pool2.execute("DELETE FROM users WHERE id=?", [userId]).catch(() => void 0);
  await pool2.end();
  await closeMysqlPool();
}
//# sourceMappingURL=integration-money-earnings.mjs.map
