...
export default function NewOrder() {
  ...
  // service_name_lowercase → completed order count (for popularity ranking)
  const [popularScores, setPopularScores] = useState<Record<string, number>>({});
  ...
  // Load SMM services + popularity scores for the selected provider
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/smm/popular-services?provider=${providerId}`)
        .then((r) => r.ok ? r.json() : { scores: {} })
        .then((d) => d.scores as Record<string, number>)
  ...
  const visibleServices = useMemo(() => {
    ...
    // Sort: services with at least 1 completed order float to the top (desc by score).
    // Within each group (popular / non-popular) original provider order is preserved.
    return [...filtered].sort((a, b) => {
      const sa = popularScores[a.name.trim().toLowerCase()] ?? 0;
      const sb = popularScores[b.name.trim().toLowerCase()] ?? 0;
      return sb - sa;
    });
  }, [services, activePlatform, activeCategory, search, popularScores]);
                ...
                <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                  {visibleServices.length === 0 ? (
                    ...
                    visibleServices.map((s) => {
                      ...
                      const popularCount = popularScores[s.name.trim().toLowerCase()] ?? 0;
                      return (
                        <button
                          key={s.service}
                          onClick={() => {
                          ...
                          <div className="flex justify-between items-start gap-3 mb-2">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-start gap-1.5 flex-wrap">
                                {popularCount > 0 && (
                                  <span className="inline-flex items-center gap-0.5 shrink-0 px-1.5 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-700 border border-orange-300">
                                    🔥 {popularCount} commande{popularCount > 1 ? "s" : ""}
                                  </span>
                                )}
                                {s.featured && (
