...
export default function Deposit() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch("/api/payments/countries");
        const data = await r.json();
        if (cancelled) return;
        if (!r.ok) {
          setCountriesError(data?.error || "Impossible de charger la liste des pays");
        } else {
          // Filtre défensif GN+CD côté UI en plus du filtre serveur.
          const all = (data.countries || []) as Country[];
          setCountries(all.filter((c) => !["GN", "CD"].includes(String(c.code).toUpperCase())));
        }
      } catch (e: any) {
        if (!cancelled) setCountriesError(e?.message || "Erreur réseau");
      } finally {
