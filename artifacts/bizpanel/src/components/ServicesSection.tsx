...
const platforms = [
  {
    ...
    description: "Abonnés, likes, commentaires et vues pour vos posts et Reels.",
  },
  {
    name: "TikTok",
    icon: TikTokIcon,
    ...
    description: "Abonnés, likes et vues pour augmenter votre portée sur TikTok.",
  },
  {
    name: "Facebook",
    icon: Facebook,
    ...
    description: "J'aime, commentaires et partages pour vos pages et publications.",
  },
  {
    name: "YouTube",
    icon: Youtube,
    ...
    description: "Abonnés, vues et likes pour développer votre chaîne YouTube.",
  },
];

const ServicesSection = () => {
  const { get } = useSiteContent();
  const sectionTitle = get("services_title", "Services par plateforme");
      ...
      <div className="container px-4 md:px-6">
        <div className="text-center mb-6 md:mb-16">
          ...
          <p className="text-muted-foreground text-xs md:text-lg max-w-2xl mx-auto">
            Instagram, TikTok, Facebook, YouTube — commandez directement depuis votre tableau de bord.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          {platforms.map((platform) => {
            ...
            return (
              <div
                key={platform.name}
                className="bg-white rounded-xl md:rounded-2xl p-3 md:p-6 shadow-sm hover:shadow-md transition-shadow hover:-translate-y-0.5 transition-transform group"
              >
                <div
                  className="w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center mx-auto mb-2 md:mb-4 text-white overflow-hidden [&_svg]:w-5 [&_svg]:h-5 md:[&_svg]:w-8 md:[&_svg]:h-8"
                  style={{ background: platform.gradient }}
                >
                  {customIcon ? (
                    <img src={customIcon} alt={platform.name} className="w-full h-full object-cover" loading="lazy" />
                  ) : (
                    <platform.icon />
                  )}
                </div>
