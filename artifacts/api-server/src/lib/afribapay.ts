...
// ---------------------------------------------------------------------------
// Countries cache — AfribaPay's /v1/countries reflects live operator
// availability (an operator that goes offline disappears or gets a
// status/enabled=false flag). We cache for a short window so a recovered
// operator reappears in the UI within ~1 minute, but high traffic doesn't
// hammer their API.
// ---------------------------------------------------------------------------
const COUNTRIES_TTL_MS = 60_000;
let countriesCache: { value: CountryEntry[]; expiresAt: number } | null = null;
let countriesInflight: Promise<CountryEntry[]> | null = null;

export function bustCountriesCache(): void {
  countriesCache = null;
  countriesInflight = null;
}

/**
 * Decide if an operator advertised by AfribaPay is currently usable.
 * AfribaPay uses several flag names depending on the endpoint version:
 *   - `enabled` / `active` / `available` (boolean or 0/1/"1")
 *   - `status` ("active", "online", "available", "down", "disabled", ...)
 * When NO flag is present, default to AVAILABLE (preserves historical
 * behaviour — the operator wouldn't be in the response otherwise).
 */
function isOperatorAvailable(op: any): boolean {
  const truthy = (v: unknown) => v === true || v === 1 || v === "1" || v === "true";
  const falsy = (v: unknown) => v === false || v === 0 || v === "0" || v === "false";

  if (op == null) return false;

  // Explicit positive flags win.
  if (op.enabled !== undefined && falsy(op.enabled)) return false;
  if (op.active !== undefined && falsy(op.active)) return false;
  if (op.available !== undefined && falsy(op.available)) return false;
  if (op.is_active !== undefined && falsy(op.is_active)) return false;
  if (op.is_enabled !== undefined && falsy(op.is_enabled)) return false;
  // Boolean truthy short-circuits (in case other fields are missing)
  ...
  if (status) {
    const okStates = new Set(["active", "available", "online", "up", "enabled", "operational", "ok"]);
    const koStates = new Set([
      "inactive", "unavailable", "offline", "down", "disabled",
      "suspended", "maintenance", "closed", "ko",
    ]);
    if (koStates.has(status)) return false;
    if (okStates.has(status)) return true;
    // Unknown status string → don't hide the operator (fail-open).
  }

  return true;
}

export async function listCountries(forceRefresh = false): Promise<CountryEntry[]> {
  // Cache hit
  ...
  const run = (async (): Promise<CountryEntry[]> => {
...
async function fetchCountriesFresh(): Promise<CountryEntry[]> {
  ...
  const normalized: CountryEntry[] = rows.map((row: any) => {
    ...
    return {
      ...
      currency,
      operators: ops
        .filter((op: any) => isOperatorAvailable(op))
        .map((op: any) => ({
...
export async function listAllowedCountries(forceRefresh = false): Promise<CountryEntry[]> {
  const all = await listCountries(forceRefresh);
  return all
    .filter((c) => !isCountryExcluded(c.code))
    .map((c) => ({
      ...c,
...
export interface PayinResponse {
  ...
  status?: string;
  message?: string;
  /** Checkout URL returned by Wave operators — open in Wave app to confirm */
  redirect_url?: string;
  raw?: unknown;
}
...
export async function payin(params: PayinParams): Promise<PayinResponse> {
  ...
  try {
    ...
    // Circuit breaker : échec côté opérateur/AfribaPay → masquer l'opérateur.
    if (err instanceof AfribapayApiError) {
      ...
      // Erreurs réseau (ECONNREFUSED, ETIMEDOUT, ...) → opérateur/infra suspect
      markOperatorFailure(params.country, params.operator, err.message);
    }
    throw err;
  }
  ...
  const inner = data?.data ?? data;
  const status = inner?.status || data?.status || undefined;
  const message = inner?.message || data?.message || undefined;

  // Le payload peut être HTTP 200 mais signaler un échec opérateur (ex.
  // status="FAILED" avec message d'indisponibilité). On l'attrape ici.
  ...
  if (statusUpper === "FAILED" || statusUpper === "ERROR") {
    ...
    // Succès (PENDING / SUCCESS) — l'opérateur répond, on nettoie tout cooldown.
    markOperatorOk(params.country, params.operator);
  }

  // Wave operators return a checkout/redirect URL to open in the Wave app
  const redirectUrl: string | undefined =
    inner?.payment_url || inner?.checkout_url || inner?.redirect_url ||
    inner?.wave_url || inner?.paymentUrl || inner?.checkoutUrl ||
    data?.payment_url || data?.checkout_url || data?.redirect_url || undefined;
  ...
  return {
    ...
    status,
    message,
    redirect_url: typeof redirectUrl === "string" && redirectUrl.startsWith("http") ? redirectUrl : undefined,
    raw: data,
  };
}
