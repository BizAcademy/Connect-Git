...
router.post("/admin/deposits/:id/credit-bonus", requireUser, requireAdmin, async (_req,res) => res.status(409).json({error:"Le crédit de bonus doit être traité par le service de paiement"}));
  ...
  router.get("/admin/transactions", requireUser, requireAdmin, async (req,res) => { try { const limit=String(req.query.limit)==="all"?100000:Math.min(Math.max(Number(req.query.limit)||200,1),1000), offset=Math.max(Number(req.query.offset)||0,0), type=String(req.query.type||"all"); const [rows]=await getMysqlPool().query<RowDataPacket[]>(`SELECT * FROM (SELECT CONCAT('o-',o.id) id,o.id local_order_id,'order' kind,o.created_at,o.charge_minor amount,o.status,o.refunded_at,o.user_id,COALESCE(p.username,p.email,o.user_id) user_label,p.email user_email,CONCAT_WS(' · ',o.service_category,o.service_name) detail,COALESCE(o.external_order_id,o.provider_order_id) reference,o.provider FROM orders o LEFT JOIN profiles p ON p.user_id=o.user_id UNION ALL SELECT CONCAT('p-',x.id),NULL,'deposit',x.created_at,x.amount_minor,x.status,NULL,x.user_id,COALESCE(p.username,p.email,x.user_id),p.email,CONCAT('Dépôt · ',COALESCE(x.method,'')),COALESCE(x.transaction_id,x.order_id,x.provider_reference),NULL FROM payments x LEFT JOIN profiles p ON p.user_id=x.user_id) t ${type==="all"?"":`WHERE kind='${type === "adjustment" ? "deposit" : type}'`} ORDER BY created_at DESC LIMIT ? OFFSET ?`,[limit,offset]); return res.json({rows:rows.map(r=>({...r,amount:Number(r.amount) / 100})),total_count:null,has_more:rows.length===limit}); } catch (err) {logger.error({err},"transactions");return res.status(500).json({error:"Erreur lecture transactions"});} });
