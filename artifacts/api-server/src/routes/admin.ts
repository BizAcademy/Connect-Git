...
import { loadPricing, setEntry, deleteEntry, enrichServices } from "../lib/smm-pricing";
...
router.get("/admin/earnings", requireUser, requireAdmin, async (req, res) => {
  try {
    ...
    const MAX_DAYS = 3650; // hard cap to avoid runaway responses

    // ---- Resolve requested window for the daily series ---------------
    ...
    let windowEnd: Date = new Date(toQ.getFullYear(), toQ.getMonth(), toQ.getDate());

    if (wantAll) {
      if (all.length > 0) {
        // Earliest earning day
        const minTs = all.reduce((min, r) => {
          const t = new Date(r.ts).getTime();
          return t < min ? t : min;
        }, Date.now());
        const minDate = new Date(minTs);
        windowStart = new Date(minDate.getFullYear(), minDate.getMonth(), minDate.getDate());
      } else {
    ...
    // Projections based on a daily rate over the rolling last 30 days.
...
router.put("/admin/providers/:id", requireUser, requireAdmin, async (req: AuthedRequest, res) => {
  ...
  if (b["display_order"] !== undefined && b["display_order"] !== null && b["display_order"] !== "") {
    ...
    if (!Number.isFinite(n) || (n !== 1 && n !== 2 && n !== 3)) {
      return res.status(400).json({ error: "display_order invalide (1, 2 ou 3)" });
    }
