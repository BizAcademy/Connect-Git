...
import {
  ...
  uploadOperatorLogoFile,
} from "../lib/operator-logos";
import { listAllowedCountries } from "../lib/afribapay";
...
const logoUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (/^image\/(png|jpeg|jpg|gif|svg\+xml|webp)$/.test(file.mimetype)) {
      ...
      cb(new Error("Type de fichier non supporté — utilisez PNG, JPG, SVG ou WebP"));
    }
  },
});
...
/**
 * PostgREST applies a maximum page size even when a larger limit is requested.
 * Read every page explicitly so admin reports do not silently stop at the
 * first 500/1000 rows.
 */
async function fetchAllRestRows<T>(
  buildUrl: (offset: number) => string,
  headers: Record<string, string>,
  label: string,
  isOptionalMissingResource = (body: string) => false,
): Promise<T[] | null> {
  const rows: T[] = [];
...
router.get("/admin/transactions", requireUser, requireAdmin, async (req: AuthedRequest, res) => {
  ...
  const limit = requestedAll
    ...
    : Math.min(Math.max(parseInt(q.limit || "200", 10) || 200, 1), 1000);
  ...
  // Read all pages from the underlying tables before merging and filtering.
  // Otherwise a search for an older order can never find it.
  const buildOrderUrl = (pageOffset: number) => {
  ...
  const buildPayUrl = (pageOffset: number) => {
    const p = new URLSearchParams();
  ...
  const buildRefUrl = (pageOffset: number) => {
    const p = new URLSearchParams();
  ...
  try {
    ...
    const [orderRows, paymentRows, referralRows] = await Promise.all([
      wantOrders
        ? fetchAllRestRows(buildOrderUrl, headers, "orders")
        : Promise.resolve([] as any[]),
      wantPays
        ? fetchAllRestRows(buildPayUrl, headers, "payments")
        : Promise.resolve([] as any[]),
      wantComs
        ? fetchAllRestRows(
            buildRefUrl,
...
router.get("/admin/users", requireUser, requireAdmin, async (req: AuthedRequest, res) => {
  ...
  const q = req.query as Record<string, string | undefined>;
  const search = (q.search || "").replace(/[%,()]/g, "").trim();
  const limit = Math.min(Math.max(parseInt(q.limit || "100", 10) || 100, 1), 2000);
  const offset = Math.max(parseInt(q.offset || "0", 10) || 0, 0);
...
// ---------------------------------------------------------------------------
// Operator logos — admin CRUD
// GET  /api/admin/operator-logos         → list all operators + logo URLs
// PUT  /api/admin/operator-logos/:code   → save/update a logo URL
// DELETE /api/admin/operator-logos/:code → remove custom logo (revert to default)
// ---------------------------------------------------------------------------
...
router.get("/admin/operator-logos", requireUser, requireAdmin, async (_req, res) => {
  try {
    const [logos, countries] = await Promise.all([
      fetchOperatorLogos(),
      listAllowedCountries().catch(() => [] as Awaited<ReturnType<typeof listAllowedCountries>>),
    ]);
    // Deduplicate operators across all countries
    const seen = new Map<string, { code: string; name: string; countries: string[] }>();
...
router.put("/admin/operator-logos/:code", requireUser, requireAdmin, async (req: AuthedRequest, res) => {
  const code = String(req.params["code"] ?? "").trim();
  const logoUrl = String(req.body?.logo_url ?? "").trim();

  if (!code) return res.status(400).json({ error: "Code opérateur requis" });
...
router.delete("/admin/operator-logos/:code", requireUser, requireAdmin, async (req: AuthedRequest, res) => {
  const code = String(req.params["code"] ?? "").trim();
  if (!code) return res.status(400).json({ error: "Code opérateur requis" });
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    return res.status(503).json({ error: "Configuration serveur manquante" });
...
// POST /api/admin/operator-logos/:code/upload — upload an image file to Supabase Storage
router.post(
