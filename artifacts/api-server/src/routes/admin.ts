...
router.post("/admin/earnings/backfill", requireUser, requireAdmin, async (_req, res) => {
  try {
    ...
    const [orders] = await getMysqlPool().query<RowDataPacket[]>("SELECT id,user_id,COALESCE(provider_order_id,external_order_id) external_id,provider,charge_minor,revenue_fcfa_minor,quantity,service_name,created_at FROM orders WHERE status='completed' AND COALESCE(provider_order_id,external_order_id) IS NOT NULL");
    let inserted = 0, skipped = 0;
    for (const o of orders) { const key = `${o.provider}:${o.external_id}`; if (existing.has(key)) { skipped++; continue; } const revenueFcfa = Number(o.revenue_fcfa_minor ?? o.charge_minor) / 100; const gain = estimateGainFromRevenue(revenueFcfa); await appendEarning({ ts: new Date(o.created_at).toISOString(), provider_order_id: String(o.external_id), user_id: o.user_id, service: 0, service_name: o.service_name || "", quantity: Number(o.quantity), rate_usd: 0, user_price_fcfa: revenueFcfa, provider_cost_usd: 0, ...gain, provider: Number(o.provider) }); inserted++; }
...
router.get("/admin/transactions", requireUser, requireAdmin, async (req,res) => {
  try {
    const limit=String(req.query.limit)==="all"?100000:Math.min(Math.max(Number(req.query.limit)||200,1),1000);
    const offset=Math.max(Number(req.query.offset)||0,0);
    const type=String(req.query.type||"all");
    const userId=String(req.query.user_id||"").trim();
    const clauses:string[]=[];
    const args:unknown[]=[];
    if(type!=="all"){clauses.push("kind=?");args.push(type==="adjustment"?"deposit":type);}
    if(userId){
    ...
    const [rows]=await getMysqlPool().query<RowDataPacket[]>(
      `SELECT * FROM (
        SELECT
          ...
          CONCAT_WS(' · ',o.service_category,o.service_name) detail,
        ...
        SELECT
          ...
          o.user_id,COALESCE(p.username,p.email,o.user_id) user_label,p.email user_email,
          CONCAT('Remboursement · ',CONCAT_WS(' · ',o.service_category,o.service_name)) detail,
