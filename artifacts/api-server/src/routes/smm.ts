...
router.post("/smm/order", requireUser, rateLimitOrders, async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    const pricePerK =
      typeof override?.price_fcfa === "number" ? override.price_fcfa : defaultPriceFcfa(svc.rate);
    ...
    if (providerData?.error) {
      ...
      await refundBalance(userId, userToken, totalPrice);
      return res.status(502).json({ error: providerData.error || "Le fournisseur n'a pas accepté la commande" });
    }
...
router.get("/smm/quote", requireUser, async (req, res) => {
  ...
  try {
    ...
    const pricePerK = typeof override?.price_fcfa === "number" ? override.price_fcfa : defaultPriceFcfa(svc.rate);
...
export async function syncOrderInternal(opts: {
  ...
  const order = rows[0]!;
  const providerId = (order.provider === 1 || order.provider === 2 || order.provider === 3 || order.provider === 4)
...
function parseProviderQuery(q: unknown): ProviderId | null {
  const n = Number(q);
  if (n === 1 || n === 2 || n === 3 || n === 4) return n as ProviderId;
  return null;
}
...
router.post("/admin/orders/:externalId/cancel", requireUser, requireAdmin, async (req: AuthedRequest, res) => {
  ...
  if (!externalId) return res.status(400).json({ error: "externalId requis" });
  const providerId = parseProviderQuery(req.query["provider"]);
  if (!providerId) return res.status(400).json({ error: "provider requis (1, 2, 3 ou 4)" });
