...
// ---------------------------------------------------------------------------
// GET /api/smm/popular-services?provider=N
// Public. Returns a score map { service_name_normalized → completed_count }
// built from the local orders table. Used by the order form to bubble up
// proven services to the top of the list.
// In-memory cache (10 min) — exact counts are not critical.
// ---------------------------------------------------------------------------
const popularCache: Map<number, { ts: number; scores: Record<string, number> }> = new Map();
...
router.get("/smm/popular-services", async (req, res) => {
  ...
  try {
    // Fetch completed orders for this provider (last 2000 to bound memory).
    const url =
    ...
    const rows = (await r.json()) as Array<{ service_name: string | null }>;

    // Group by normalized service name (lowercase+trim for robust matching).
