...
// ---------------------------------------------------------------------------
router.get("/payments/countries", async (req, res) => {
  try {
    // ?refresh=1 forces a fresh call to AfribaPay (bypasses the 60 s cache).
...
router.post("/payments/initiate", requireUser, async (req: AuthedRequest, res) => {
  ...
  // 2% processing fee charged to the user on top of the credit amount.
  // `amount`      = what the user receives (stored in DB, used for balance credit).
  // `chargeAmount` = what AfribaPay actually deducts from the user's mobile money.
  const FEE_RATE = 0.02;
  ...
  try {
    const result = await payin({
      ...
      country,
      phone_number: phone,
      amount: chargeAmount,   // charge amount — what AfribaPay deducts
      currency,
      order_id: orderId,
    ...
    return res.json({
      ...
      wave_redirect_url: result.redirect_url || undefined,
...
router.post("/payments/webhook", async (req: Request & { rawBody?: string }, res) => {
  ...
  try {
    // Webhook reçu = l'opérateur a répondu : on nettoie tout cooldown
    // associé pour qu'il réapparaisse immédiatement aux autres utilisateurs.
    const opCode = String(local["operator"] ?? "");
    const opCountry = String(local["country"] ?? "");
    if (opCode && opCountry && isSuccessStatus(rawStatus)) {
      try {
        const { markOperatorOk } = await import("../lib/operator-health");
        markOperatorOk(opCountry, opCode);
      } catch { /* best effort */ }
    }
...
// ---------------------------------------------------------------------------
// GET /api/payments/operator-logos — public, no auth needed
// Returns a map of operatorCode → logo URL for all operators that have a
// custom logo configured by the admin.
// Cache HTTP agressif (5 min frais + 1 h stale-while-revalidate) car les
// logos changent très rarement et sont chargés sur chaque visite de la home.
// ---------------------------------------------------------------------------
router.get("/payments/operator-logos", async (_req, res) => {
  try {
