...
--
-- This migration:
--   1. Drops the legacy CHECK (1..4) constraint.
...
--   4. Adds a new CHECK constraint accepting only id 1, 3, 4, 5.
--   5. Renumbers `display_order` to be contiguous 1..4 across the four remaining
--      providers, preserving their relative order.
--
-- Idempotent: safe to re-run.

-- 1. Drop the old CHECK constraint FIRST so the DELETE/INSERT below can run
ALTER TABLE smm_providers_config
  DROP CONSTRAINT IF EXISTS smm_providers_config_provider_id_check;

-- 2. Remove provider 2 (GROWFOLLOWERS) BEFORE adding the new constraint
...
-- 4. Now the table only contains valid ids — add the new CHECK constraint
ALTER TABLE smm_providers_config
  ADD CONSTRAINT smm_providers_config_provider_id_check
  CHECK (provider_id IN (1, 3, 4, 5));

-- 5. Renumber display_order to a contiguous 1..N sequence
WITH ranked AS (
  SELECT
    provider_id,
